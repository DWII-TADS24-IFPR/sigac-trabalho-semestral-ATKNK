    ...
    <div class="container mt-4">
        ...
        <form action="{{ route('documento.update', $documento->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="url" name="url" class="form-control" value="{{ old('url', $documento->url) }}" required>
            </div>

            <div class="mb-3">
                <label for="descricao" class="form-label">Descrição:</label>
                <input type="text" name="descricao" class="form-control" value="{{ old('descricao', $documento->descricao) }}" required>
            </div>

            <div class="mb-3">
                <label for="horas_in" class="form-label">Horas Inseridas:</label>
                <input type="number" name="horas_in" class="form-control" step="0.1" min="0" value="{{ old('horas_in', $documento->horas_in) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="comentario" class="form-control">{{ old('comentario', $documento->comentario) }}</textarea>
            </div>

            <div class="mb-3">
                <label for="horas_out" class="form-label">Horas Validadas (opcional):</label>
                <input type="number" name="horas_out" class="form-control" step="0.1" min="0" value="{{ old('horas_out', $documento->horas_out) }}">
            </div>
